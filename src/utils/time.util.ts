...
class TimeUtil {
	static isOutRangeOfYear(year: IPicker.DatePicker['year']) {
		return year < DATE_PICKER.YEARS[0] || year > DATE_PICKER.YEARS[DATE_PICKER.YEARS.length - 1];
	}

	static isMaxAvailableYear(month: IPicker.DatePicker['month'], year: IPicker.DatePicker['year']) {
		return month >= MONTHS_IN_YEAR && year >= DATE_PICKER.YEARS[DATE_PICKER.YEARS.length - 1];
	}

	static isMinAvailableYear(month: IPicker.DatePicker['month'], year: IPicker.DatePicker['year']) {
	...
	static daysInMonth(month: IPicker.DatePicker['month'], year: IPicker.DatePicker['year']): number {
	...
	static generateCalendar(date: IPicker.DatePicker): IPicker.DatePicker['day'][] {
	...
	static toTimePickerText(time?: IPicker.TimePicker): string {
		...
		return `${NumberUtil.toZeroPrefix(hour)}:${NumberUtil.toZeroPrefix(minute)} ${suffix}`;
	}
}
